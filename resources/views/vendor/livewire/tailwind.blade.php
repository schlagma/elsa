            ...
            <div class="flex justify-between flex-1 sm:hidden">
                <span>
                    @if ($paginator->onFirstPage())
                        <button disabled class="btn-neutral">
                            <span aria-hidden="true">@svg('mdi-chevron-left', 'size-5 -ml-0.5')</span>
                            {!! __('pagination.previous') !!}
                        </button>
                    @else
                        <button type="button" wire:click="previousPage('{{ $paginator->getPageName() }}')" x-on:click="{{ $scrollIntoViewJsSnippet }}" wire:loading.attr="disabled" dusk="previousPage{{ $paginator->getPageName() == 'page' ? '' : '.' . $paginator->getPageName() }}.before" class="btn-neutral">
                            <span aria-hidden="true">@svg('mdi-chevron-left', 'size-5 -ml-0.5')</span>
                            {!! __('pagination.previous') !!}
                        </button>
                    @endif
                ...
                <span>
                    @if ($paginator->hasMorePages())
                        <button type="button" wire:click="nextPage('{{ $paginator->getPageName() }}')" x-on:click="{{ $scrollIntoViewJsSnippet }}" wire:loading.attr="disabled" dusk="nextPage{{ $paginator->getPageName() == 'page' ? '' : '.' . $paginator->getPageName() }}.before" class="btn-neutral">
                            {!! __('pagination.next') !!}
                            <span aria-hidden="true">@svg('mdi-chevron-right', 'size-5 -mr-0.5')</span>
                        </button>
                    @else
                        <button disabled class="btn-neutral">
                            {!! __('pagination.next') !!}
