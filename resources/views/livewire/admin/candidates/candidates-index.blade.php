<div class="p-6 sm:p-8 overflow-y-auto h-full">
    <div class="xl:flex xl:items-center">
        ...
        <div class="mt-4 xl:ml-16 xl:mt-0 flex flex-col sm:flex-row whitespace-nowrap">
            <a wire:navigate href="{{ route('admin-candidates-add') }}" type="button" class="btn-primary">
    ...
    <div class="mt-6 mb-8 grid sm:grid-cols-2 md:grid-cols-3 xl:grid-cols-5 gap-6">
        <div>
            ...
            <input type="text" id="filter-lastname" wire:model.live="lastname">
        </div>
        <div>
            <label for="filter-firstname">{{ __('admin.lastname') }}</label>
            <input type="text" id="filter-firstname" wire:model.live="firstname">
        </div>
        <div>
            <div>
        ...
        <div>
            <div>
                ...
                <select id="filter-faculty" wire:model.change="faculty" title="{{ __('admin.faculty') }}">
            ...
            <div class="inline-block min-w-full py-2 align-middle sm:px-6 lg:px-8">
                ...
                <div class="overflow-hidden shadow-sm ring-1 ring-black/5 sm:rounded-lg">
                    <table>
                        <thead>
                            <tr>
                                <th scope="col" class="min-w-[12rem]">{{ __('admin.lastname') }}</th>
                                <th scope="col" class="min-w-[12rem]">{{ __('admin.firstname') }}</th>
                                <th scope="col" class="min-w-[9rem]">{{ __('admin.election') }}</th>
                                <th scope="col" class="min-w-[14rem]">{{ __('admin.faculty') }}</th>
                                <th scope="col" class="min-w-[14rem]">{{ __('admin.committee') }}</th>
                                <th scope="col" class="min-w-[8rem]">{{ __('admin.approvedShort') }}</th>
                        ...
                        <tbody>
                            ...
                            <tr>
                                ...
                                <td>@if(app()->getLocale() == "de"){{ json_decode($candidate->election)[0] }}@elseif(app()->getLocale() == "en"){{ json_decode($candidate->election)[1] }}@endif</td>
                                <td>@if(app()->getLocale() == "de"){{ json_decode($candidate->faculty)[0] }}@elseif(app()->getLocale() == "en"){{ json_decode($candidate->faculty)[1] }}@endif</td>
                                <td>@if(app()->getLocale() == "de"){{ json_decode($candidate->committee)[0] }}@elseif(app()->getLocale() == "en"){{ json_decode($candidate->committee)[1] }}@endif</td>
                                <td>@if($candidate->approved)<span aria-hidden="true">@svg('mdi-check', 'size-6 text-green-600 dark:text-green-500')</span><span class="sr-only">{{ __('admin.yes') }}</span>@else<span aria-hidden="true">@svg('mdi-close', 'size-6 text-red-600 dark:text-red-300')</span><span class="sr-only">{{ __('admin.no') }}</span>@endif</td>
                                <td class="whitespace-nowrap text-right">
                                    <div class="relative inline-block text-left" x-data="{ dropdown: false }">                                            
                                        ...
                                        @if ($loop->index < count($candidates) - 2)
                                        ...
                                        @endif
                                            <div class="py-1">
                                                <a wire:navigate href="{{ route('admin-candidates-edit', ['id' => $candidate->id]) }}" class="text-zinc-800 dark:text-white group flex items-center px-4 py-2 border-0 hover:bg-zinc-200 dark:hover:bg-zinc-700 !font-normal">
