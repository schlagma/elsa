<div class="flex flex-col p-6 sm:p-8 pb-0! overflow-y-auto h-full">
    <div class="xl:flex xl:items-center">
        <div class="sm:flex-auto">
            <h1 class="text-2xl font-bold leading-6 text-zinc-800 dark:text-white">{{ __('admin.committeesEditTitle') }}</h1>
        </div>
    </div>
    <div class="mt-6 mb-12">
        <div class="grid sm:grid-cols-2 gap-6">
            <flux:field>
                ...
                <flux:input type="text" wire:model="nameDE" />
            </flux:field>
            <flux:field>
                <flux:label>{{ __('admin.nameEN') }}</flux:label>
                <flux:input type="text" wire:model="nameEN" />
            </flux:field>
        </div>
        ...
        <div class="mt-6 grid sm:grid-cols-2 xl:grid-cols-3 gap-6">
            <flux:field>
                ...
                <flux:input type="number" wire:model="seats" />
            </flux:field>
            <flux:field>
                <flux:label>{{ __('admin.seatsDeputy') }}</flux:label>
                <flux:input type="number" wire:model="seatsDeputy" />
            </flux:field>
            <flux:field>
        ...
        <div class="mt-6 grid sm:grid-cols-2 xl:grid-cols-3 gap-6">
            ...
            <flux:field>
                <flux:label>{{ __('admin.priority') }}</flux:label>
                <flux:input type="number" wire:model="priority" />
            </flux:field>
        </div>
    </div>
